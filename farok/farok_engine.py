...
def eff_burn(self):
    if self.has_burn !=0:
        RETURN(f"화상으로 {self.has_burn} 피해 입음")
...
def eff_bleed(self):
    if self.has_bleed !=0:
        RETURN(f"출혈으로 {self.has_bleed} 피해 입음")
        self.hp -= self.has_bleed
        self.has_bleed -= 1

def eff_blight(self):
    if self.has_blight !=0:
        RETURN(f"중독으로 {self.dmg_blight} 피해 입음")
...
def eff_corrosion(self):
    if self.has_corrosion !=0:
        RETURN(f"부식으로 {self.has_corrosion} 피해 입음")
...
def eff_guard(self):
    if self.has_guard != 0:
        RETURN(f"{self.guarded_by}에게 {self.has_guard}동안 보호받음")
        self.has_guard -= 1
        RETURN(f"남은 보호: {self.has_guard}")
    if self.has_guard == 0:
        RETURN(f"{self.guarded_by}에게 받는 보호 해제됨.")
...
def apply_bleed(self, chance, amount, target):
    if target.has_res == 0:
        ...
        if ch >= (100-(chance-target.bleedres)):
            ...
            RETURN(f"대상 출혈을 {amount}만큼 변경.\n"
                  f"현재 출혈량 = {target.has_bleed}\n")
    else:
        RETURN(f"저항으로 인해 효과 차단됨. \n"
    ...
    if target.has_res == 0:
        if target.has_blight == 0:
            ...
            if ch >= (100 - (chance - target.blightres)):
                ...
                RETURN(f"대상 중독을 {amount}만큼 변경.\n"
                      f"현재 중독량 = {target.dmg_blight}\n")
        else:
            RETURN("대상이 이미 중독에 걸려 있어 중독 추가 불가능.")
    else:
        RETURN(f"저항으로 인해 효과 차단됨. \n"
              f"현재 남은 저항: {target.has_res}\n")
        target.has_res -= 1
...
def apply_crit(self, chance, amount, target):
    ...
    if ch >= (100-chance):
        ...
        RETURN(f"대상 치명타 토큰을 {amount}만큼 변경.\n"
...
def apply_prot(self, chance, amount, target):
    ...
    if ch >= (100-chance):
        ...
        RETURN(f"대상 방어 토큰을 {amount}만큼 변경.\n"
...
def apply_dodge(self, chance, amount, target):
    ...
    if ch >= (100-chance):
        ...
        RETURN(f"대상 회피 토큰을 {amount}만큼 변경.\n"
...
def apply_power(self, chance, amount, target):
    ...
    if ch >= (100-chance):
        ...
        RETURN(f"대상 힘 토큰을 {amount}만큼 변경.\n"
...
def useinitiative(self):
    self.initiative -= 1
    RETURN(f"남은 행동권: {self.initiative}")
...
def dmgcalc(self, target, damage, iW, iP):
    ...
    if self.has_bleed !=0:
        df_mod -= 0.1
        RETURN(f"공격자 출혈으로 인해 가하는 피해 10% 차감, 남은 출혈 {self.has_bleed}")
    if self.has_weak !=0:
        if ignore_weak == False:
            df_mod -= 0.15
            RETURN(f"공격자 무력 토큰으로 인해 가하는 피해 15% 차감, 남은 무력 토큰 {self.has_weak}")
            self.has_weak -= 1
        if ignore_weak == True:
            RETURN(f"공격자 무력 토큰 무시, 남은 무력 토큰 {self.has_weak}")
    if target.has_vuln !=0:
        df_mod += 0.15
        RETURN(f"대상 취약 토큰으로 인해 가하는 피해 15% 증가, 남은 취약 토큰 {target.has_vuln}")
        target.has_vuln -= 1
    if target.has_protplus != 0:
        if ignore_prot == True:
            target.has_protplus -=1
            RETURN(f"돌파 공격으로 인해 대상 방어+ 토큰 무시 및 차감, 남은 방어+ 토큰 {target.has_protplus}")
        if ignore_prot == False:
            df_mod -= 0.75
            target.has_protplus -= 1
            RETURN(f"대상 방어+ 토큰으로 인해 피해 75% 차감, 남은 방어토큰 {target.has_protplus}")
    elif target.has_prot != 0:
        if ignore_prot == True:
            target.has_prot -= 1
            RETURN(f"돌파 공격으로 인해 대상 방어 토큰 무시 및 차감, 남은 방어 토큰 {target.has_prot}")
        if ignore_prot == False:
            df_mod -= 0.5
            target.has_prot -= 1
            RETURN(f"대상 방어 토큰으로 인해 피해 50% 차감, 남은 방어토큰 {target.has_prot}")
    if self.has_power != 0:
        df_mod += 0.25
        self.has_power -= 1
        RETURN(f"공격자 힘 토큰으로 인해 피해 25% 증가, 남은 힘 토큰 {self.has_power}")
    if target.has_sinking !=0:
        RETURN(f"침잠으로 {target.has_sinking} 정신 피해 입힘")
    ...
    if self.has_crit != 0:
        df_dmg *= 1.5
        self.has_crit -= 1
        RETURN(f"치명타! (남은 치명타 토큰: {self.has_crit}")
    ceil(df_dmg)
    return df_dmg
#명중률 알고리즘
    ...
    if stp != target.immunity:
        ...
        if self.has_blind != 0:
            if ignore_blind == True:
                RETURN(f"공격자 실명 토큰 무시, 남은 실명 토큰 {self.has_blind}")
            if ignore_blind == False:
                df_accmod -= 0.25
                RETURN(f"공격자 실명 토큰으로 인해 명중률 25% 감소, 남은 실명 토큰 {self.has_blind}")
        if target.has_dodgeplus != 0:
            if ignore_dodge == True:
                target.has_dodgeplus -= 1
                RETURN(f"추적 공격으로 인해 대상 회피+ 토큰 무시 및 차감, 남은 회피+ 토큰 {target.has_dodgeplus}")
            if ignore_dodge == False:
                df_accmod -= 0.75
                RETURN(f"대상 회피+ 토큰으로 인해 명중률 75% 감소, 남은 회피+ 토큰 {target.has_dodgeplus}")
        elif target.has_dodge != 0:
            if ignore_dodge == True:
                target.has_dodge -= 1
                RETURN(f"추적 공격으로 인해 대상 회피 토큰 무시 및 차감, 남은 회피 토큰 {target.has_dodge}")
            if ignore_dodge == False:
                df_accmod -= 0.5
                RETURN(f"대상 회피 토큰으로 인해 명중률 50% 감소, 남은 회피 토큰 {target.has_dodge}")
...
def statusplayer(self):
    ...
    RETURN(f"현재 상태: {self.status}\n"
          ...
          f"현재 가진 상태이상/토큰:\n")
    if self.has_bleed !=0:
        RETURN(f"출혈: {self.has_bleed}\n")
    if self.has_blight != 0:
        RETURN(f"중독: {self.has_blight}\n")
    if self.has_corrosion != 0:
        RETURN(f"부식: {self.has_corrosion}\n")
    if self.has_burn != 0:
        RETURN(f"화상: {self.has_burn}\n")
    if self.has_vuln != 0:
        RETURN(f"취약: {self.has_vuln}\n")
    if self.has_weak != 0:
        RETURN(f"무력: {self.has_weak}\n")
    if self.has_capture != 0:
        RETURN(f"속박: {self.has_capture}\n")
    if self.has_blind != 0:
        RETURN(f"실명: {self.has_blind}\n")
    if self.has_horror != 0:
        RETURN(f"공포: {self.has_horror}\n")
    if self.has_sinking != 0:
        RETURN(f"침잠: {self.has_sinking}\n")
    if self.has_crit != 0:
        RETURN(f"치명타: {self.has_crit}\n")
    if self.has_power != 0:
        RETURN(f"힘: {self.has_power}\n")
    if self.has_res != 0:
        RETURN(f"저항: {self.has_res}\n")
    if self.has_guarded != 0:
        RETURN(f"{self.guarded_by}에게 보호받음: {self.has_guard}\n")
    if self.has_prot != 0:
        RETURN(f"방어: {self.has_prot}\n")
    if self.has_protplus != 0:
        RETURN(f"방어+: {self.has_protplus}\n")
    if self.has_dodge != 0:
        RETURN(f"회피: {self.has_dodge}\n")
    if self.has_dodgeplus != 0:
        RETURN(f"회피+: {self.has_dodgeplus}\n")
    if self.has_restoration != 0:
        RETURN(f"재생: {self.has_restoration}\n")
    if self.has_resonance != 0:
        RETURN(f"공명 중첩: {self.has_resonance}")
    if self.has_explode != 0:
        RETURN(f"폭발 중첩: {self.has_explode}")
    if self.has_fatigue != 0:
        RETURN(f"피로 중첩: {self.has_fatigue}")
    if self.immunity != None:
        RETURN(f"현재 {im} 피해에 면역 상태임")
...
def guardcalc(precalctarget,tlist,isFriendly):
    ...
    if isFriendly == False:
        if t1.has_taunt != 0:
            RETURN(f"도발로 인해 {t1.myID}에게 강제 공격")
            target = t1
            target.has_taunt -= 1
        elif t2.has_taunt != 0:
            RETURN(f"도발로 인해 {t2.myID}에게 강제 공격")
            target = t2
            target.has_taunt -= 1
        elif t3.has_taunt != 0:
            RETURN(f"도발로 인해 {t3.myID}에게 강제 공격")
            target = t3
            target.has_taunt -= 1
        elif t4.has_taunt != 0:
            RETURN(f"도발로 인해 {t4.myID}에게 강제 공격")
            target = t4
            target.has_taunt -= 1
        if precalctarget.guarded_by == t1.myID:
            RETURN(f"보호로 인해 {t1.myID}에게 강제 공격")
            target = t1
        elif precalctarget.guarded_by == t2.myID:
            RETURN(f"보호로 인해 {t2.myID}에게 강제 공격")
            target = t2
        elif precalctarget.guarded_by == t3.myID:
            RETURN(f"보호로 인해 {t3.myID}에게 강제 공격")
            target = t3
        elif precalctarget.guarded_by == t4.myID:
            RETURN(f"보호로 인해 {t4.myID}에게 강제 공격")
            target = t4
    return target
...
def playerskillinput(self,tlist,plist):
    ...
    if move == 7:
        ...
        if self.minmove <= amount <= self.maxmove:
            ...
            RETURN(f"이동: 현재 {self.in_rank}열에 있습니다.")
    if move == 8:
